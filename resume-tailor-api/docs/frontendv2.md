...
Keep `frontend.md` as the canonical reference for legacy routes; use this addendum only for the new resume-section editing surface.

The studio currently reads `resume.insight` on `GET /resumes` payloads. That field is not returned — map `extractedSummary` / `skills` / `experience` instead. See `../resume-tailor-frontend/docs/studio-data-shapes.md`.
