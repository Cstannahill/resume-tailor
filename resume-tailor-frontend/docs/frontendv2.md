...
- Server prompts are enriched with the user's resume summary/skills, indexed project highlights, and persona-coach insights, so clients only need to send UI-specific deltas.
...
Keep `docs/frontend.md` as the canonical reference for legacy routes; use this addendum only for the resume-section editing and developer-report surfaces. Update your client services accordingly.
