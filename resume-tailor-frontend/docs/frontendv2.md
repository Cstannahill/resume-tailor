...
- Server prompts are auto-enriched with resume summary/skills, indexed project highlights, and persona-coach insights, so the UI only needs to send section-specific deltas.
...
Keep `frontend.md` as the canonical reference for route-level integration. Use this addendum for resume-section editing and developer-report workflows. Update your client services accordingly.
