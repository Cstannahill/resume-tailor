...
- Server-side prompt enrichment uses the shared [user context pipeline](../../resume-tailor-api/docs/user-context.md), so the UI only needs to send task-specific deltas.
...
- The backend automatically gathers resume highlights, persona insights, and project summaries through the [user context pipeline](../../resume-tailor-api/docs/user-context.md) before asking the LLM.
- Response shape:
  ```json
  {
    "data": {
