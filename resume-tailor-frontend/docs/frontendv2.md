...
- Server auto-enriches prompts with resume summary/skills, indexed project highlights, and persona-coach insights, so clients only need to send UI-specific deltas.
...
- Resume section JSON parse failures still return 200 with `content` fallback (raw text) and `rationale` describing parse issues.
