...
The studio currently reads `resume.insight` on `GET /resumes` payloads. That field is not returned — map `extractedSummary` / `skills` / `experience` instead. See [`studio-data-shapes.md`](./studio-data-shapes.md).
